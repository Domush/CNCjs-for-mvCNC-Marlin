...
const Result = ({ triangle, jogValues, onBack, onClose }) => {
  ...
  const checkStepsPerMM = () => {
    ...
    const calculatedXStep = Math.round(
      movementCalculation({ currentStep: Number(currentYStep), movedDistance: y, actualDistance: a })
    );
    const calculatedYStep = Math.round(
      movementCalculation({ currentStep: Number(currentXStep), movedDistance: x, actualDistance: b })
    );
      ...
      return (
        <>
          <div className={styles.result}>
            <Icon type="warning" />
            <p>
              Your machine is pretty squared, but if you still want to have it full squared, move either the right
              y-axis rail backwards <strong>{fm}mm</strong> or the left y-axis rail forward by <strong>{fm}mm</strong>
            </p>
          </div>
          ...
          {stepsUpdate ? (
            <>
              <p className={styles.resultText}>
                We also noticed your steps-per-mm can be updated for improved accuracy on your machine, you can click
    ...
    return (
      <>
        <div className={styles.result}>
          <Icon type="error" />
          <p>
            Your machine is off by <strong>{fm}mm</strong>, to properly square it, move either the right y-axis rail
            backwards <strong>{fm}mm</strong> or the left y-axis rail forward by <strong>{fm}mm</strong>
          </p>
        </div>
        ...
        {stepsUpdate ? (
          <>
            <p className={styles.resultText}>
              We also noticed your steps-per-mm can be updated for improved accuray on your machine, you can click the
