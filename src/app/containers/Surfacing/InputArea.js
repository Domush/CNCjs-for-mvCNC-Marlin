...
const InputArea = ({ values, onChange, onSelect, units }) => {
  ...
  const { bitDiameter, stepover, feedrate, length, width, skimDepth, spindleRPM, maxDepth, type, startPosition } =
    values;

  const defaultValues =
    units === METRIC_UNITS ? widgets.surfacing.defaultMetricState : widgets.surfacing.defaultImperialState;
