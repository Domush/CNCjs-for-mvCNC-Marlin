...
export class GCodeProcessor {
  ...
  syncStateToMachine(options = {}) {
    ...
    if (shouldInclude('feed')) {
      vmState.feed =
        controller.feed || (Array.isArray(this.options.maxFeed) ? this.options.maxFeed[0] : this.options.maxFeed);
    }
  ...
  runGcodeLine(gline) {
    ...
    let isSimpleMotion =
      ...
      (gline.has(motionCode) ? 1 : 0) + (gline.has('F') ? 1 : 0) + (gline.has('N') ? 1 : 0) + hasCoords.length ===
        gline.words.length;
